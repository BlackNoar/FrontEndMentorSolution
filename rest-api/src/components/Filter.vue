...
<template>
  <div class="flex py-5 justify-end gap-5 flex-wrap md:justify-between items-center">
    <!-- Search Input -->
    <div
    ...
    <div @click.self="open" class="relative">
      <button
        @click="open = !open"
        class="flex flex-row dark:bg-dark-card dark:text-white items-center w-52 px-6 py-3 text-sm font-semibold text-left bg-white rounded-lg"
      >
      ...
      <div
        v-show="open"
        class="absolute z-50 right-0 w-full mt-2 origin-top-right rounded-md shadow-lg md:w-48"
      >
        <div
        ...
        >
          <button
            ...
            :key="btn.id"
            @click="data.handleSelect(`${btn.filter}`)"
